...
struct UserDetail: HandyJSON {
    ...
    var userAuthInfo: UserAuthInfo? { return UserAuthInfo.deserialize(from: user_auth_info) }
}
...
struct UserDetailDongtai: HandyJSON {
    ...
    var detailHeaderHeight: CGFloat? {
        // 60 + contentHeight + origin_thread.height + 25 + 5 + 40
        var height: CGFloat = 130 + contentH!
        switch item_type {
    ...
    var cellHeight: CGFloat? {
        // 顶部分割线 5，头像用户名 45，底部 50， 中间部分上下间距 5
        var height: CGFloat = 110 + contentH!
        switch item_type {
        case .postVideoOrArticle, .postVideo, .answerQuestion, .proposeQuestion, .forwardArticle, .postContentAndVideo:   // 发布了视频和文章,提出了问题,回答了问题
            height += 50
        case .postContent, .postSmallVideo:   // 发布了文字内容
...
struct DongtaiOriginThread: HandyJSON {
    /// cell 的高度
    var height: CGFloat? { return 20 + contentH! + (isDongtaiDetail ? detailCollectionViewH! : collectionViewH!) }
    
    var content: String = ""
    var contentH: CGFloat? {
        let height = Calculate.textHeight(text: content, fontSize: 17, width: screenWidth - 30.0) + 5.0
