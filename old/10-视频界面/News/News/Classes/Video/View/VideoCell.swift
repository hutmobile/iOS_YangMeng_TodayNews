...
class VideoCell: UITableViewCell, RegisterCellFromNib {
    ...
    var video = NewsModel() {
        didSet {
            ...
            if video.label_style == 3 { // 说明是广告
                commentButton.setTitle("查看详情", for: .normal)
                commentButton.theme_setImage("images.view detail_ad_feed_15x13_", forState: .normal)
                commentButton.theme_setTitleColor("colors.userDetailSendMailTextColor", forState: .normal)
            } else {
